...
mydb=connection.MySQLConnection(user='root',host='localhost',password='Dss@1023',db='ecom28db')
...
def myorder_details(ordid):
    ...
    try:
        ...
        print(f'Order Details Error',str(2))
        return jsonify({'status':'failed','message':str(e)}),500
    finally:
        if cursor:
            cursor.close()
...
def get_invoice(ord_id):
    ...
    try:
        ...
        userid=session.get('userid')
        #fetch all orders
        cursor.execute('''select orderid,razorpay_ordid,razorpay_payment,total_amount,delivery,
                    tax,grand_total,created_at from orders where userid=uuid_to_bin(%s) and  orderid=%s
                    ''',[userid,ord_id])
        order_data=cursor.fetchone()
        if not order_data:
            return jsonify({'status':'failed','message':'order not found'}),404
        cursor.execute('''select item_name,item_price,item_quantity,subtotal,item_category,
                    item_filename from order_items where orderid=%s''',[ord_id])
        orders_itemsdata=cursor.fetchall()
        #-------------- create pdf buffer----------
        pdf_buffer=BytesIO()
        #---------------------Create Document
        ...
        #-----------------Order details
        order_info=f"""<b>ORDERID : </b>{order_data[0]}<br/>
                        <b>RAZORPA Y ORDERID : </b>{order_data[1]}<br/>
                        <b>PAYMENTID : </b>{order_data[2]}<br/>
                        <b>ORDER DATE: </b>{order_data[7]}<br/>"""
        order_para=Paragraph(order_info,styles['BodyText'])
        elements.append(order_para)
        elements.append(Spacer(1,10))
        elements.append(HRFlower(width="100%"))
        elements.append(Spacer(1,15))

        #------------------- Table data-----------
        table_data=[['Item Name','Category','Price','Quantity','Subtotal']]
        for item in orders_itemsdata:
            table_data.append([item[0],item[4],f'₹{float(item[1])}',item[2],f"₹{float(item[3])}"])
        ...
        <b>Tax : </b>₹{float(order_data[4])}<br/><br/>
        <b>Grand Total: </b> ₹{float(order_data[6])}<br/><br/>"""
        summary_para=Paragraph(summary,styles['Heading3'])
        elements.append(summary_para)
        elements.append(Spacer(1,25))
        #-------------------------Footer-------------------
        footer=Paragraph('Thankyou for shopping with BUYROUTE',styles['ITALIC'])
...
def buy_now():
    ...
    try:
        ...
        if quantity > available_stock:
            return jsonify({'status':'failed','message':'Insufficient stock'}),400
        img_url=url_for('static',filename=f'uploads/{item_data[7]}',_external=True)
        #------stroing single buy item in session temp
        session['single_buy']={
        ...
        return jsonify({
            'status':'success','message':'Single buy item stored successfully','payment_type':'single','item':{
                'itemid':item_data[0],
                'itemname':item_data[1],
...
def descitem(itemid):
    ...
    try:
        ...
        if not item_data:
            return jsonify({'status':'failed','message':'Item not found'}),404
        item_details={'itemid':item_data[0],
                ...
                'category':item_data[6],
                'image':url_for('static', filename=f'uploads/{item_data[7]}',_external=True),
                # 'created_at':item_data[8].isoformat()
            }
        return jsonify({'status':'success','item':item_details}),200
    except Exception as e:
        print("VIEW ITEMS ERROR:",str(e))
        return jsonify({'status':'failed','message':str(e)}),500
    finally:
